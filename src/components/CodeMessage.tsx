import AudioPlayer from "../pages/CodeInput/AudioPlayer";
...
const CodeMessage = ({
  onClose,
  unlockedMessage,
  closeButtonText = "Đã hiểu rồi",
}: CodeMessageProps) => {
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-[1000] animate-fade-in p-4">
      <div className="bg-[#FFF8DC] border-2 border-[#FFDAB9] max-h-[95vh] max-w-2xl w-full relative animate-scale-in shadow-2xl">
        <button
        ...
        <div className="px-4 md:px-12 py-4 md:py-8 text-center overflow-y-auto h-full">
          ...
          <h2 className="text-xl md:text-2xl lg:text-3xl font-serif font-bold text-[#2C2C2C] tracking-tight pb-2">
            THÔNG ĐIỆP <br />CHO EM
          </h2>

          <div className="flex items-center justify-center mb-4">
            <div className="flex-1 h-px bg-[#FFDAB9]" />
            <div className="mx-4 w-2 h-2 bg-[#FFDAB9] rounded-full" />
            <div className="flex-1 h-px bg-[#FFDAB9]" />
          </div>

          <div className="space-y-6 text-[#4A4A4A] mx-2">
            <div className="text-sm text-justify md:text-base leading-relaxed max-h-[50vh] overflow-y-auto">
            ...
            <div>
              <button
                className="mt-2 text-base bg-[#FFDAB9] hover:bg-[#FFC8A2] text-[#2C2C2C] py-2 px-6 font-serif font-semibold tracking-wider transition-colors duration-300 shadow-md hover:shadow-lg"
                onClick={onClose}
              >
                {closeButtonText}
              </button>
